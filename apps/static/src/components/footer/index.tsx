...
export default function DefaultFooter() {
  ...
  return (
    <footer className="m-auto w-full max-w-screen-xl space-y-8 px-4 py-16 xl:px-0">
      <div className="flex-col items-center justify-between md:flex md:flex-row">
            ...
            <div className="flex gap-4">
              {icons.map((k, i) => (
                <Link
                  ...
                  href={"/"}
                  className={cn(
                    "border-light-green bg-light-green/10 rounded-md border-2 p-2 [&_svg]:size-4"
                  )}
                >
                  {k.icon}
                </Link>
        ...
        <div className="grid w-full max-w-md flex-1 grid-cols-2 gap-x-8 gap-y-4 pt-8 md:flex md:justify-center md:py-0 md:text-lg">
          {links.map((l, i) => (
            <Link key={i} href={l.url} className="inline-block w-fit">
              {l.title}
            </Link>
          ))}
          {isMobile &&
            commonLinks.map((l, i) => (
              <Link key={i} href={l.url} className="inline-block w-full">
                {l.title}
              </Link>
            ))}
        </div>
      ...
      <div className="justify-between text-sm md:flex md:text-base">
        {!isMobile && (
          <div className="text-muted-foreground flex w-full max-w-screen-sm gap-12">
            {commonLinks.map((l, i) => (
              <Link key={i} href={l.url} className="block w-fit">
                {l.title}
              </Link>
            ))}
          </div>
        )}
        <p className="mt-auto text-center md:text-start">
          © Copyright 2023 Mangohr
        </p>
      </div>
    </footer>
  )
